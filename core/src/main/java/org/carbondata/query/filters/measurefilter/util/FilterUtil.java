...
public final class FilterUtil {
	...
	private static void traverseAndResolveTree(
			FilterResolverIntf filterEvaluatorTree,
			AbsoluteTableIdentifier tableIdentifier) {
		if (null == filterEvaluatorTree) {
			return;
		}
		traverseAndResolveTree(filterEvaluatorTree.getLeft(), tableIdentifier);

		filterEvaluatorTree.resolve(tableIdentifier);

		traverseAndResolveTree(filterEvaluatorTree.getRight(), tableIdentifier);
	}

	private static FilterResolverIntf createFilterResolverTree(
			Expression expressionTree, AbsoluteTableIdentifier tableIdentifier) {
	...
	private static FilterResolverIntf getFilterResolverBasedOnExpressionType(
		...
		case EQUALS:
			...
			if (currentCondExpression.isSingleDimension()
				...
				if (currentCondExpression.getColumnList().get(0).getDim()
						.isNoDictionaryDim()) {
					if (checkIfExpressionContainsColumn(currentCondExpression
							.getLeft())
	...
	public static FilterExecuter getFilterExecuterTree(
			FilterResolverIntf filterExpressionResolverTree,
			KeyGenerator blockKeyGenerator) {
		return createFilterExecuterTree(
